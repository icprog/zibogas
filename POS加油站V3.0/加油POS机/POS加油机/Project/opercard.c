...
{	
	...
	fBuf = (double)(cardinfo->capacity / 100.00);
	sprintf(strbuf, "升  数: %5.2lf", fBuf);
	EA_vDisp(3, 1, strbuf);
	...
	EA_vDisp(2, 1, strbuf);
	EA_vDisp(4, 1, "金  额: %4.2lf元", temp_fare);
...
//需要更新白名单
	...
	{
		if(EA_ucBlkClean ((char *)blackFileName) != EM_SUCCESS)
		{
			lcddisperr("清空白名单失败");
			Beeperr();
			GPRS_Close();

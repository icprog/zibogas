...
//  var_bcd2asc(buf+buf_len, (uchar *)&prec->capacity, 4);
	sprintf((char *)(buf+buf_len), (char *)"%08d", cardinfo->capacity);
	buf_len+=8;
#ifdef _zb_debug_tran_
...
reboot:
	...
	sscanf((void *)input, "%f", &temp_cap);
	
	if( DevStat.fill_mode == 0 )
	{
		sprintf((void *)buf, "加气量:%4.2f立方", temp_cap);
	}
	else
	{
		sprintf((void *)buf, "加油量:%4.2f升  ", temp_cap);
	}

	CardInfo.capacity = temp_cap * 100;
	CardInfo.fare = temp_cap * 	CardInfo.price;
	...
	if ( temp_int32u >= 0xF423F )   //每天消费大于999999
		temp_int32u = 0;
	else
		temp_int32u++;
...
//  Debugprintf(CardInfo.bus_number);						 		//车自编号
	...
	//保存卡类型
	CardInfo.card_type = block_buffer1[11];													 //卡类型
...
//  SleepMs(2000);
	...
	if (CardInfo.card_type != 0xAA)                 //只允许普通卡消费debug  by frank
	{

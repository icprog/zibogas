...
void print_record (RECORD *prec)
{
	...
	if( prec->fuel_type == 0 )
	{
	fBuf= (float)(prec->capacity)/100;
	sprintf((char*)strbuff, "   加气体积：%6.2f立方\n", fBuf);
	(void)EA_ucLinePrinter(hDprinterHandle, 0, (char*)strbuff);
	}
	else
	{
	fBuf= (float)(prec->capacity)/100;
	sprintf((char*)strbuff, "   加油体积：%6.2f升\n", fBuf);
	(void)EA_ucLinePrinter(hDprinterHandle, 0, (char*)strbuff);
	}

	//加油余额
	fBuf= (float)(prec->fare)/100;
	sprintf((char*)strbuff, "   消费金额：%6.2f元\n", fBuf);
	(void)EA_ucLinePrinter(hDprinterHandle, 0, (char*)strbuff);

	//油品单价
	fBuf = (float)(prec->price)/100.0;
	sprintf((char*)strbuff, "   油品单价：%6.2f元\n", fBuf);
	(void)EA_ucLinePrinter(hDprinterHandle, 0, (char*)strbuff);
...
//消费次数
	...
	(void)EA_ucLinePrinter(hDprinterHandle, 0, (char*)strbuff);
//加油体积
	sprintf((char*)strbuff, "  天然气体积 ：%6.2f立方\r\n", (float)((DevStat.binhand.consume_cap[0])/100.0));
	(void)EA_ucLinePrinter(hDprinterHandle, 0, (char*)strbuff);
	sprintf((char*)strbuff, "  汽油93#体积：%6.2f升\r\n", (float)((DevStat.binhand.consume_cap[1])/100.0));
	(void)EA_ucLinePrinter(hDprinterHandle, 0, (char*)strbuff);
	sprintf((char*)strbuff, "  汽油97#体积：%6.2f升\r\n", (float)((DevStat.binhand.consume_cap[2])/100.0));
	(void)EA_ucLinePrinter(hDprinterHandle, 0, (char*)strbuff);
	sprintf((char*)strbuff, "  柴 油 体 积：%6.2f升\r\n", (float)((DevStat.binhand.consume_cap[3])/100.0));
	(void)EA_ucLinePrinter(hDprinterHandle, 0, (char*)strbuff);
	sprintf((char*)strbuff, "-----------------------------------------\n");
	(void)EA_ucLinePrinter(hDprinterHandle, 0, (char*)strbuff);
	//消费金额
	sprintf((char*)strbuff, "  天然气金额 ：%6.2f元\r\n", (float)((DevStat.binhand.consume_amt[0])/100.0));
	(void)EA_ucLinePrinter(hDprinterHandle, 0, (char*)strbuff);
	sprintf((char*)strbuff, "  汽油93#金额：%6.2f元\r\n", (float)((DevStat.binhand.consume_amt[1])/100.0));
	(void)EA_ucLinePrinter(hDprinterHandle, 0, (char*)strbuff);
	sprintf((char*)strbuff, "  汽油97#金额：%6.2f元\r\n", (float)((DevStat.binhand.consume_amt[2])/100.0));
	(void)EA_ucLinePrinter(hDprinterHandle, 0, (char*)strbuff);
	sprintf((char*)strbuff, "  柴 油 金 额：%6.2f元\r\n", (float)((DevStat.binhand.consume_amt[3])/100.0));
	(void)EA_ucLinePrinter(hDprinterHandle, 0, (char*)strbuff);
	sprintf((char*)strbuff, "-----------------------------------------\n");
	(void)EA_ucLinePrinter(hDprinterHandle, 0, (char*)strbuff);

	sprintf((char*)strbuff, "  总  金  额 ：%6.2f元\r\n", (float)((DevStat.binhand.consume_amt[0]+DevStat.binhand.consume_amt[1]+

...
// 功能：		参数设置界面
// *****************************************************************
INT8U  parm_set[11*20 +2] = {"1.更新名单          2.油价调整          3.油品设置          4.记录查询          5.设备号设置        6.站点号设置        7.密码修改          8.重打上次小票      9.IP地址设置        10.修改系统时间     11.修改加油卡密码   "};
	...
	switch ( ret )
	{
	   case 0:	// 自动更新
		...
		case 1:	// 手动设置
			...
			sscanf((void *)input, "%f", &temp_price);
...
void AnalyAndDisplay(RECORD * strTradeInfo)
{
	...
	//显示卡片信息
	EA_vCls();
	htoa(temp_buf, &prec->bus_number[0], 3);
	strcpy((void *)strbuf, (void *)"车号:");
	...
	{
		case 0:
			sprintf((void *)strbuf, "油品:天燃气");//检索卡片类型信息
			EA_vDisp(2,1, strbuf);

			fBuf    = (float)(prec->fare)/100.0;
			sprintf((void *)strbuf, "金额:%5.2f元   ", fBuf);
			EA_vDisp(3,1, strbuf);
			fBuf    = (float)(prec->capacity)/100.0;
			sprintf((void *)strbuf, "升数:%5.2f立方 ", fBuf);
			EA_vDisp(4,1, strbuf);
			break;
		case 1:
			sprintf((void *)strbuf, "油品:汽油93#");//检索卡片类型信息
			EA_vDisp(2,1, strbuf);
			...
			EA_vDisp(3,1, strbuf);
			fBuf    = (float)(prec->capacity)/100.0;
			sprintf((void *)strbuf, "升数:%5.2f升   ", fBuf);
			EA_vDisp(4,1, strbuf);
			break;
		case 2:
			sprintf((void *)strbuf, "油品:汽油97#");//检索卡片类型信息
			EA_vDisp(2,1, strbuf);
			...
			break;
		case 3:
			sprintf((void *)strbuf, "油品:柴油");//检索卡片类型信息
			EA_vDisp(2,1, strbuf);

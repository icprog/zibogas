...
//  	return;
	...
	sprintf((char *)strbuff, (char *)"欢迎使用淄博公交IC卡\n");
...
//	mtclprn_charline_print("\x1Bw1\x1BW0  ABCDEFGH\r\r");  
	...
	switch ( prec->card_purchase_type)
	{
		...
		//卡片原金额
			...
			sprintf((char*)strbuff, "卡片原额：%6.2f元\n", fBuf);
//  		mtclprn_charline_print(strbuff);
			(void)EA_ucLinePrinter(hDprinterHandle, 0, (char*)strbuff);
			break;
		case TT_CREDIT_MONEY:				
...
//  memcpy(RecBuff, &prec->fuel_type, 1);
	...
	{
		sprintf((char*)strbuff, "燃    料：汽油\n");
	}
	(void)EA_ucLinePrinter(hDprinterHandle, 0, (char*)strbuff);

//卡片类型
//  GetCardType(prec->card_type, RecBuff);
//  sprintf((char*)strbuff, "\x1Bw1\x1BW1卡片类型: %s\r\r", RecBuff);
//  (void)EA_ucLinePrinter(hDprinterHandle, 0, (char*)strbuff);
...
//	sprintf((char*)strbuff, "售卡押金：%6.2f元\n", fBuf);
//  mtclprn_charline_print(strbuff);
//  (void)EA_ucLinePrinter(hDprinterHandle, 0, (char*)strbuff);
...
//  sprintf((char*)strbuff, "大卡余额:%8.2f元\n", fBuf);
//  mtclprn_charline_print(strbuff);
//    (void)EA_ucLinePrinter(hDprinterHandle, 0, (char*)strbuff);
////收入汇总
//    sprintf((char*)strbuff, "收入汇总:%8.2f元\n", (float)( DevStat.binhand.credit_amt - DevStat.binhand.flush_amt)/100);
////  mtclprn_charline_print(strbuff);
//    (void)EA_ucLinePrinter(hDprinterHandle, 0, (char*)strbuff);
///------------------------
//  mtclprn_charline_print("\x1Bw1\x1BW1------------------------------\r\r"); //  \x1BW1打印XYZ\x1Bw0打印\x1BW0打印1234567890这里超长不会打出!\r\r\r");
	sprintf((char*)strbuff, "-----------------------------------------\n");
	(void)EA_ucLinePrinter(hDprinterHandle, 0, (char*)strbuff);
	
//消费次数
	sprintf((char*)strbuff, "加油次数:%6d次\r\n", DevStat.binhand.consume_cnt);
	(void)EA_ucLinePrinter(hDprinterHandle, 0, (char*)strbuff);

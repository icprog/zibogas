...
INT32U uihisrecLen = sizeof(_BANLANCE_IN_HAND);
...
{
	//建立7个记录文件包含，参数表，黑名单，本次记录、
	//正常补采记录、历史记录、灰记录, 员工名单
	INT8U i = 0;
	//重新参数表
	...
	{
		//重新创建黑名单文件
		i = EA_ucBlkCreate((char *)blackFileName, uiblackLen, 20000);
		if ( i == EM_bfs_FILE_EXIST )          //文件已经存在
		{
			...
			if (EA_ucBlkCreate((char *)blackFileName, uiblackLen, 20000) == EM_SUCCESS )
			{
				lcddisperr("重新创建白名单");
			}
		}
		...
		//重新创建记录文件
		 i = EA_ucPFCreateLoop((uchar *)currecFileName, uicurrecLen, 2000);  //2000条循环记录;
		 if ( i == EM_ffs_FILEEXIST )          //文件已经存在
		 {
		 ...
		 //重新创建历史记录文件
		 i = EA_ucPFCreateLoop((uchar *)hisrecFileName, uihisrecLen, 500);
		 if ( i == EM_ffs_FILEEXIST )          //文件已经存在
		 {
			 ...
			 if ( EA_ucPFCreateLoop((uchar *)hisrecFileName, uihisrecLen, 500) == EM_ffs_SUCCESS )
			 {
				 lcddisperr("重建历史记录");
			 }
		 }
